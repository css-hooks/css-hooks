...
- **`":<pseudo>"`**, where `<pseudo>` is any
  ...
  `hover`, `active`, or `focus`. _This is deprecated, and we recommend to use a
  selector spec instead._
...
- **`"@supports <query>"`**, where `<query>` can be replaced with any
  [feature query](https://www.w3.org/TR/css-conditional-3/#at-supports)
  condition.
...
- `sort?: boolean` &ensp; _experimental_ &ensp; When enabled, the last
  declaration specified always has the highest priority.
...
### <Code className="language-typescript">type CssFn = (...styles: WithHooks&lt;CSSProperties&gt;[]) => CSSProperties</Code>
...
`style` prop. When multiple style objects are specified, each one is flattened
in order to provide fallback values for the next.
