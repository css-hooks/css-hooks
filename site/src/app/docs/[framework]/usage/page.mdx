...
The main interface you will interact with throughout your application is that of
the `hooks` function, created via `createHooks` as shown in the
[Getting started](./getting-started) guide.

You can make use of hooks anywhere you find a `style` prop that you'd like to
enhance with state-driven styles like `hover`, `focus`, `active`, or `disabled`,
to name just a few. For example, consider the case of a simple button:

<CodeExample name="initial" framework={props.params.framework} />

You can access hooks simply by "wrapping" the style object in a `hooks` call:

<CodeExample name="hooksWrap" framework={props.params.framework} />
...
typical case that _disabled_ styles need to override other states, you should
